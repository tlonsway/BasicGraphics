...
public class Client implements Runnable{
	...
	private ClientUDPListenThread UDPListener;
	Graphics game;
			...
			while(running.bool) {
				...
				if(messages.size() > 0) {
					...
					if(command.equals("close")) {
						...
						}else if(data[1].equals("udp")) {
							if(UDPListener != null) {
								UDPListener = new ClientUDPListenThread(running, this);
								new Thread(UDPListener).start();
								sendUDPInfo();
							}
							new Thread(new ClientUDPUpdateThread(data[2], Integer.parseInt(data[3]), this, running)).start();
						}
	...
	public void sendUDPInfo() {
		...
		requests.add("g:udp:"+UDPListener.getIP()+":"+UDPListener.getPort());
	}
